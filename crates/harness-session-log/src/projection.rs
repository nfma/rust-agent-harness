...
pub(crate) fn project(
    ...
    let mut identities = HashSet::from([session_id.as_str(), user_entry_id.as_str(), operation_id]);
    if identities.len() != 3 {
        return Err(ProjectionError::Corrupt);
    }

    let terminal = match records.get(2) {
        None => None,
        Some(Record::Entry {
            entry_id,
            parent_entry_id,
            ...
            match payload {
                Payload::AssistantMessage { text, .. } => {
                    Some(ProjectedTerminal::Assistant(text.clone()))
                }
                ...
                {
                    Some(ProjectedTerminal::Failure(*failure_code))
                }
                _ => return Err(ProjectionError::Corrupt),
            }
        }
        Some(_) => return Err(ProjectionError::Corrupt),
    };
